	...
	*/
	...
	function getTableList($skip_authentication = false) {
		...
		$arrTables = array(
			...
			'Teachers_Table' => array('Teachers Table', '', 'resources/table_icons/account_balances.png', 'None'),
			'Students_Table' => array('Students Table', '', 'resources/table_icons/administrator.png', 'None')
		);
	...
	function get_sql_fields($table_name) {
		$sql_fields = array(
			'Teachers_Table' => "`Teachers_Table`.`Name` as 'Name', `Teachers_Table`.`Email_id` as 'Email_id', IF(    CHAR_LENGTH(`Teachers_Table1`.`Name`) || CHAR_LENGTH(`Teachers_Table1`.`Email_id`), CONCAT_WS('',   `Teachers_Table1`.`Name`, `Teachers_Table1`.`Email_id`), '') as 'Ph_number'",
			'Students_Table' => "`Students_Table`.`Name` as 'Name', `Students_Table`.`Email_id` as 'Email_id', IF(    CHAR_LENGTH(`Students_Table1`.`Name`), CONCAT_WS('',   `Students_Table1`.`Name`), '') as 'ph_number'",
		);
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false) {
		$sql_from = array(
			'Teachers_Table' => "`Teachers_Table` LEFT JOIN `Teachers_Table` as Teachers_Table1 ON `Teachers_Table1`.`Name`=`Teachers_Table`.`Ph_number` ",
			'Students_Table' => "`Students_Table` LEFT JOIN `Students_Table` as Students_Table1 ON `Students_Table1`.`Name`=`Students_Table`.`ph_number` ",
		);
	...
	function htmlUserBar() {
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> Cbytes database</a>
			</div>
